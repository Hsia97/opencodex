...
describe("google maxOutputTokens clamp", () => {
  ...
  test("protects thinkingBudget constraint (maxOutputTokens >= thinkingBudget) without exceeding model max", () => {
    expect(clampGoogleMaxOutputTokens("custom-model", 4000, 8000)).toBe(8000);
    expect(clampGoogleMaxOutputTokens("custom-model", 20000, 8000)).toBe(16384);
    expect(clampGoogleMaxOutputTokens("custom-model", 4000, 100000)).toBe(16384);
    expect(clampGoogleMaxOutputTokens("gemini-3.7-flash", 4000, 100000)).toBe(65536);
  });
});
