...
describe("Antigravity Multi-Signature History Stability (Mechanism ②)", () => {
  ...
  test("clamps to the latest signature when occurrences exceed recorded history", () => {
    const sessionId = "session-hist-2";
    const model = "gemini-3.7-flash";

    observeAntigravityReplay(model, sessionId, [
      fcPart("read_file", { path: "foo.txt" }, "sig-first-1234567890"),
    ]);

    observeAntigravityReplay(model, sessionId, [
      fcPart("read_file", { path: "foo.txt" }, "sig-second-123456789"),
    ]);

    // Request with 3 calls (history only has 2)
    const contents = [
      { role: "model", parts: [{ functionCall: { name: "read_file", args: { path: "foo.txt" } } }] },
      { role: "model", parts: [{ functionCall: { name: "read_file", args: { path: "foo.txt" } } }] },
      { role: "model", parts: [{ functionCall: { name: "read_file", args: { path: "foo.txt" } } }] },
    ];
    ...
    const parts = contents.map(c => (c.parts[0] as { thoughtSignature?: string }).thoughtSignature);
    expect(parts[0]).toBe("sig-first-1234567890");
    expect(parts[1]).toBe("sig-second-123456789");
    expect(parts[2]).toBe("sig-second-123456789"); // fallback to latest known
  });
